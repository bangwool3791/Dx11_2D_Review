...
public:
	void LookAt(const float4& ZAxis)
	{
	...
	void View(const float4& _EyePostion, const float4& _EyeDir, const float4& _Up)
	{
		...
		// 길이 1짜리 벡터로 만들고
		float4 Look = float4::NormalizeReturn(_EyeDir);
		...
		// 길이 1짜리 벡터로 만들고
		float4 Right = float4::Cross(_Up, Look);
		Right.Normalize();

		//XMVECTOR R1 = XMVector3Cross(R2, R0);
		// 길이가 1인 벡터 2개를 외적하면 무조건 길이 1짜리 벡터가 나온다.
		float4 Up = float4::Cross(Look, Right);
		...
		// 
		float D0Value = float4::DotProduct3D(Right, NegEyePosition);
		float D1Value = float4::DotProduct3D(Up, NegEyePosition);
		float D2Value = float4::DotProduct3D(Look, NegEyePosition);
		...
		float4x4 Mat;
		Mat.ArrV[0] = float4::Select(D0, Right, Control);
		Mat.ArrV[1] = float4::Select(D1, Up, Control);
		Mat.ArrV[2] = float4::Select(D2, Look, Control);
